...
class env_base:
    ...
    def initialization(self, **kwargs):
        ...
        for i in range(self.robot_num):
            robot = mobile_robot(id=i, mode=self.robot_mode, radius=self.robot_radius_list[i], init_state=self.init_state_list[i], goal=self.goal_list[i], step_time=self.step_time, **kwargs)
            self.robot_list.append(robot)
            self.robot = robot if i == 0 else None

        # car robot
        for j in range(self.acker_num):
            car = car_robot(id=j, length=self.acker_shape_list[j][0], width=self.acker_shape_list[j][1], wheelbase=self.acker_shape_list[j][2], wheelbase_w = self.acker_shape_list[j][3], init_state=self.acker_init_state_list[j], goal=self.acker_goal_list[j], step_time=self.step_time, vel_limit=self.acker_vl, vel_ang_limit =self.acker_val, **kwargs)
